...
export class RestClientService implements OnInit {
  ...
  register(registrationDTO: RegistrationDTO): Observable<RestMessage<string>> {
    return this.restClient('post', '/register', registrationDTO) as Observable<
      RestMessage<string>
    >;
  }

  login(loginDTO: LoginDTO): Observable<RestMessage<string>> {
    return this.restClient('post', '/login', loginDTO) as Observable<
      RestMessage<string>
    >;
  }

  logout(username: string): Observable<RestMessage<string>> {
    return this.restClient('post', '/logout', username) as Observable<
      RestMessage<string>
    >;
  }

  create(itemDTO: ItemDTO): Observable<RestMessage<ItemDTO>> {
    return this.restClient('post', '/create', itemDTO) as Observable<
      RestMessage<ItemDTO>
    >;
  }
